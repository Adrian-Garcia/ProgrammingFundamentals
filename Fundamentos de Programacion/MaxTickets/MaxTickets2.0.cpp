...
void GetValues (int iMat[][iMAXROWS], int &iNR, int &iNC) {
    ...
    //Validate that Shows are positive and less than 14
    if (iNC > 0 && iNC < 5) {
        
        cin >> iNR; //Ask for number of days
        //Validate that Shows are positive and less than 30
        if (iNR > 0 && iNR < 31) {    
...
void Display (int iMat[][iMAXROWS], int &iNR, int &iNC) {
    ...
    int iMax = iMat[0][0]; //Set the first number as the biggest one
